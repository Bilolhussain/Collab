...
export const verifyEmailToken = (userData, history) => (dispatch) => {
  axios
    .post('/api/users/verifyRegistrationEmail', userData, {
...
export const sendResetPassEmail = (userData, history) => (dispatch) => {
  axios
    .post('/api/users/sendResetPassEmail', userData, { withCredentials: true })
...
export const resendVerificationEmail = (userData, history) => (dispatch) => {
  axios
    .post('/api/users/resendVerificationEmail', userData, {
      withCredentials: true,
    })
    .then((res) => history.push(`/verifyEmail?email=${res.data.email}`))
    .catch((err) =>
...
export const resetPassUsingToken = (userData, history) => (dispatch) => {
  axios
    .post('/api/users/resetPassword', userData, { withCredentials: true })
...
export const loginWithFacebook = (userData, history) => (dispatch) => {
  axios
    .post('/api/users/loginViaFacebook', userData)
    .then((result) => {
      /**
       * Once you get the response , save the data received from result.data to localStorage
       * We are using object destructuring here, below code is equivalent to const token = result.data.token
...
export const loginWithGoogle = (userData, history) => (dispatch) => {
  axios
    .post('/api/users/loginViaGoogle', userData)
    .then((result) => {
      /**
       * Once you get the response , save the data received from result.data to localStorage
       * We are using object destructuring here, below code is equivalent to const token = result.data.token
...
export const createAssessment = (newAssessment, history) => (dispatch) => {
  axios
    .post('/api/users/assessment', newAssessment, { withCredentials: true })
    .then((assessment) => {
      const value = true;
...
//Save assessment data and set firstLogin to true
export const createMeeting = (newMeeting, history) => (dispatch) => {
  axios
    .post('/api/users/meeting', newMeeting, { withCredentials: true })
    .then((meeting) => {
      const value = true;
      dispatch(updateLogin(value));
      history.push('/dashboard');
